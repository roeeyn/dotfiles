...
function obj:start()
    self.windowFilter:subscribe(hs.window.filter.windowFocused, function(window, _)
        ...
        if
            previousFrame ~= nil
            and nextFrame.x == previousFrame.x
            and nextFrame.y == previousFrame.y
            and nextFrame.w == previousFrame.w
            and nextFrame.h == previousFrame.h
        then
            return
        end
        ...
        if previousFrame ~= nil then
            ...
            local arrowFrame =
                hs.geometry(previousFrame.x + previousFrame.w / 2 - arrowSize / 2, previousFrame.y + previousFrame.h / 2 - arrowSize / 2, arrowSize, arrowSize)
            ...
            if arrowFrameIntersection.w * arrowFrameIntersection.h == 0 then
                ...
                hs.canvas
                    ...
                    :rotateElement(1, angle)
                    :show()
                    :delete(self.arrowFadeOutDuration)
            end
        end
        ...
        hs.canvas
            ...
            :show()
            :delete(self.highlightFadeOutDuration)
    end)
end
